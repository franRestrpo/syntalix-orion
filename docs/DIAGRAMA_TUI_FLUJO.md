# Diagrama de Flujo - Syntalix-Orion TUI

## Arquitectura General de Flujo de Datos
...
flowchart TB
    ...
    %% Role execution
    site_yml -->|"include_roles:<br/>- traefik<br/>- grafana"| traefik_role
    site_yml -->|"include_roles:<br/>- grafana"| grafana_role
    site_yml -->|"include_roles: *"| other_roles

    %% Template rendering
    traefik_role -->|"template:<br/>traefik.env.j2 → traefik.env"| env_files
    traefik_role -->|"template:<br/>traefik.yml.j2 → traefik_stack.yml"| stack_files
    grafana_role -->|"template:<br/>grafana.env.j2 → grafana.env"| env_files
    grafana_role -->|"template:<br/>grafana.yml.j2 → grafana_stack.yml"| stack_files
...
## Flujo Detallado: De TUI a Archivos .env
...
sequenceDiagram
    ...
    Note over DependencyGraph: Mapea TRAEFIK__DOMAIN<br/>Mapea TRAEFIK__USER<br/>Mapea TRAEFIK__PASSWORD
    ...
    Note over Role: Busca TRAEFIK__USER en extra vars<br/>Valor: "miouser"
...
## Flujo de Variables: Format APPID__VARIABLE
...
flowchart LR
    ...
    style TEMPLATE fill:#1a1a2e,stroke:#00D9FF,color:#00D9FF
    style APP_ACTUAL fill:#0f3460,stroke:#F59E0B,color:#F59E0B
    style OUTPUT fill:#16213e,stroke:#10B981,color:#10B981
```

---

## Diagrama de States - StateStore
...
## Detalle del Role Ansible - Traefik
...
## Template traefik.env.j2 - Detalle de Renderizado
...
## Flujo Completo: Selección → Despliegue
